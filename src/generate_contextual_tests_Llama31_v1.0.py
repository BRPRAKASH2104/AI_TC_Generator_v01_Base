...
def generate_tests_with_context(requirement, heading, info_list, interface_list):
    ...
    **Logic Table Data:**
    ...
    """
    # ### THIS IS THE FIX ###
    # Use the correct model name that matches the 'ollama list' output on your system.
    response_str = call_ollama("llama3.1:8b", prompt, is_json=True)
    # ######################
...
def main():
    ...
    for reqifz_file in files_to_process:
        ...
        output_csv_path = reqifz_file.with_name(f"{reqifz_file.stem}_TCD_Llama31_Final.csv")
